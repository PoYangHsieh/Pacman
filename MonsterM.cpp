...
using namespace std;
MonsterM::MonsterM(const int a,const int b)///初始位置和pacman pointer
    : Monster(a,b)
{
    t=0;
}

MonsterM &MonsterM::move()///移動，檢查有沒有碰到
{
    t++;
    if(t%200 > 40 && t%200<=105){
    	MoveAim();
	}else if(t%200 >150 && t%200<=195){
		MoveThroughWall();
	}else{
		MoveRandom();
	}
...
MonsterM &MonsterM::MoveRandom()///移動，檢查有沒有碰到
{
    if( lose )
        return *this;
...
MonsterM &MonsterM::MoveThroughWall()
{
	...
	if(count==1){//只有一條路可以走
		...
		switch( direction ){
	        case 0:
	        	if(mazePtr->maze[x][y-2]==2 || mazePtr->maze[x][y-2]==0){
	        		y-=2;
				}else{
					--y;
				}
	        ...
	        case 1:
	            ...
	            break;
	        case 2:
	        	if(mazePtr->maze[x-2][y]==2 || mazePtr->maze[x-2][y]==0){
	        		x-=2;
				}else{
					--x;
				}
