...
using namespace std;
MonsterJ::MonsterJ(const int a,const int b)///初始位置和pacman pointer
    : Monster(a,b)
{
    t=0;
}

MonsterJ &MonsterJ::move()///移動，檢查有沒有碰到
{
    nowPositions.erase( make_pair(x,y) );
    if((x-pacmanPtr->getX())*(x-pacmanPtr->getX())+(y-pacmanPtr->getY())*(y-pacmanPtr->getY())<30)
        MoveRandom();
    else
        MoveAim();
...
MonsterJ &MonsterJ::MoveRandom()///移動，檢查有沒有碰到
{
    if( lose )
        return *this;
